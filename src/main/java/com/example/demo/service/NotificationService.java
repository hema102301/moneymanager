...
public class NotificationService {
	...
	@Scheduled(cron = "0 0 22 * * *", zone = "IST") // 10 PM IST
		 ...
		 List<ProfileEntity> profiles = profileRepo.findAll();
		    ...
		    for (ProfileEntity profile : profiles) {
		        ...
		        String body = "<html>"
		                ...
		                + "<a href="+frontendUrl+"'http://localhost:3000/login' "
	...
	@Scheduled(cron = "0 0 23 * * *", zone = "IST") // every minute for testing
	    ...
	    for (ProfileEntity profile : profiles) {
	        ...
	        if (!todayExpenses.isEmpty()) {
	            ...
	            for (ExpenseDTO expense : todayExpenses) {
	                table.append("<tr style='background-color:" + (counter % 2 == 0 ? "#f9f9f9" : "#ffffff") + ";'>")
	            ...
	            // Send
	            log.info("Sending email to: {}\nSubject: {}", profile.getEmail(), subject);
